...
export default function MathGame() {
  const {
    level, problem, userInput, score, lives, isGameOver,
    feedback, showCorrectAnimation, consecutiveCorrect,
    handleNumberClick, handleClear, handleBackspace, analytics,
  } = useMathGame();

  // Compact, animated lives
  const renderLives = () => {
    return Array.from({ length: 10 }).map((_, i) => (
      <div 
        ...
        className={`transition-all duration-300 transform ${
          i < lives 
            ? "scale-100 opacity-100 filter drop-shadow-[0_0_3px_rgba(255,215,0,0.8)]" 
  ...
  return (
    <div className="fixed inset-0 bg-[#0f172a] text-white flex flex-col p-4 select-none overflow-hidden font-sans">
      {isGameOver && <ResultOverlay score={score} analytics={analytics} level={level} />}
      ...
      {/* COMPACT LIVES BAR */}
      <div className="flex justify-center gap-0.5 py-3 bg-slate-900/30 rounded-full mt-2 border border-white/5">
        {renderLives()}
      </div>
      ...
      <div className="flex-1 flex flex-col justify-center items-center relative my-4">
        <div className="w-full aspect-video bg-slate-800 rounded-[2rem] border-4 border-slate-700 shadow-2xl flex flex-col items-center justify-center relative overflow-hidden">
          ...
          <p className="text-slate-400 text-xs font-bold uppercase mb-2 tracking-tighter">Solve This</p>
          <h2 className="text-5xl md:text-6xl font-black text-yellow-300 drop-shadow-[0_4px_0_rgba(0,0,0,0.4)]">
            {problem?.problem}
          </h2>

          {/* Correct Feedback Overlay */}
          <div className={`absolute inset-0 bg-emerald-500 flex items-center justify-center transition-transform duration-300 ${showCorrectAnimation ? 'translate-y-0' : 'translate-y-full'}`}>
             <span className="text-7xl"><CheckIcon/></span>
          </div>
        </div>
        ...
        <div className={`
          ...
          ${feedback === "wrong" ? "bg-red-600 border-red-800 animate-shake" : "bg-indigo-600 border-indigo-800"}
        `}>
